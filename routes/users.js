...
router.get('/index', function (req, res) {
	Topic.printTopics(req, res, function (item) {
		res.render('index', { layout: 'layout', topic: item });
	});
});
...
router.get('/home', ensureAuthenticated, function (req, res) {
	Topic.printTopics(req, res, function (item) {
		res.render('home', { layout: 'layout', topic: item });
	});
});
		...
		Answer.printMultipleAnswers(req, res, function (item1) {
			Reply.printReply(req, res, function (item2) {
				Topic.printTopicByID(req, res, topicId, function (item3) {
					res.render('topic', { layout: 'layout', items: item, ans: item1, topic: item3.content, topicID: topicId, comm: item2 });
				});
			});
		});
	});
...
router.post('/topic', function (req, res) {
	...
	req.flash('success_msg', 'Your comment posted successfully');
...
router.post('/ask', function (req, res) {
	...
	Topic.findTotal(req, res, function (count) {
		...
		if (!topicArray[0]) {
			Topic.printTopics(req, res, function (item) {
				res.render('ask', { topic: item, error1: 'Please select atleast 1 topic' });
			});
		}
		else if (errors) {
			Topic.printTopics(req, res, function (item) {
...
router.post('/answer', function (req, res) {
	...
	if (errors) {
		Question.getQuestionByQid(req, res, tempQid, function (item) {
			res.render('answer', { tempQuestion: item, layout: 'layout', errors: errors });
		});
	}
	else {
